...
module.exports = class InteractionCreateEvent extends BaseEvent {
  ...
  async run(client, interaction) {
    ...
    if (interaction.customId == "claim4") {
      if (!interaction.member.roles.cache.has(roel4)) return
if(await client.n38th.get(interaction.channelId)) return interaction.reply({content : "التكت مستلمة بالفعل",ephemeral : true})
      ...
      interaction.channel.permissionOverwrites.edit(roel4, {
        ViewChannel: false
      })
 ...
 - تـم ترك تـذكـرتـك مـن قـبـل الإداري  : __** ${interaction.member}`)
  ...
  `)
    ...
    if (interaction.customId == "delete4") {
      if (!interaction.member.roles.cache.has(roel4)) return
